...
- **Actualizar:** Cuando estado **<= 3**: el usuario aporta o modifica datos del comprobante (entidad, productos, montos, tipo doc, moneda), envía datos en JSON, o expresa intención de iniciar/registrar una venta o compra. Estado 3 significa que todos los obligatorios están llenos pero el usuario aún puede corregir cualquier dato antes de confirmar. Si estado >= 4 no clasificar como actualizar (a partir de estado 4 las condiciones de actualizar son las de opciones). **Campos posibles en actualizar (Estado 1–3):** operacion (venta/compra), entidad_nombre, entidad_numero, tipo_documento (factura/boleta/nota de venta/nota de compra), moneda (PEN/USD), metodo_pago (contado/credito; método de pago), dias_credito, nro_cuotas, monto_total, monto_sin_igv, igv, productos (array con nombre, cantidad, precio), fecha_emision, fecha_pago. A partir de estado 4 los campos de opciones: sucursal, forma_pago, centro_costo solo compra.
