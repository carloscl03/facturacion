...
class AnalizadorService:
    ...
    def ejecutar(self, wa_id: str, mensaje: str, id_from: int) -> dict:
        ...
        try:
            ...
            texto_registrado = (
                resumen_visual
                or "¿Los datos son correctos? Indique si desea confirmar o modificar algo."
            ).strip()
    ...
    def _construir_payload(self, propuesta: dict, estado_actual: dict, contexto_previo: str | None) -> dict:
        ...
        return {
            ...
            or estado_actual.get("entidad_id_tipo_documento"),
            ...
            "monto_impuesto": float(
                propuesta.get("monto_impuesto") or estado_actual.get("monto_impuesto") or 0
            ),
        ...
        base_estado = _sin_nulos(
            {
                ...
                **(
                    {"monto_total": estado_actual.get("monto_total")}
                    if estado_actual.get("monto_total")
                    and float(estado_actual.get("monto_total") or 0) > 0
                    else {}
                ),
                **(
                    {"monto_base": estado_actual.get("monto_base")}
                    if estado_actual.get("monto_base")
                    and float(estado_actual.get("monto_base") or 0) > 0
                    else {}
                ),
                **(
                    {"monto_impuesto": estado_actual.get("monto_impuesto")}
                    if estado_actual.get("monto_impuesto")
                    and float(estado_actual.get("monto_impuesto") or 0) > 0
                    else {}
                ),
        ...
        nuevo_parcial = _sin_nulos(
            {
                ...
                **(
                    {"monto_total": monto_total_nuevo}
                    if monto_total_nuevo and float(monto_total_nuevo) > 0
                    else {}
                ),
                **(
                    {"monto_base": monto_base_nuevo}
                    if monto_base_nuevo and float(monto_base_nuevo) > 0
                    else {}
                ),
                **(
                    {"monto_impuesto": monto_impuesto_nuevo}
                    if monto_impuesto_nuevo and float(monto_impuesto_nuevo) > 0
                    else {}
                ),
